...
-- create database Weather_Observation_Station;

use Weather_Observation_Station;
...
insert into station(ID, CITY, STATE, LAT_N)  value
(144, 'Eskridge', 'KS', 1 );
...
select( 
(select count(city) from station) 		/*This is total number CITY in the table*/
		- 	
(select count(city) from (select distinct city from station) as tb)	)
										/*This is number diffrent CITY in the table*/
...
with tb as (select distinct CITY, left(CITY,1) as lf from STATION)
select CITY from tb
WHERE lf in ("A", "E", "I", "O", "U")
...
greater than 38.788 and less than 137.2345. Truncate your answer to 4 decimal places.*/

select round(sum(Lat_N), 4) as sun_latN from station
...
Set @b = (SELECT Min(LONG_W) FROM STATION WHERE LONG_W != 0);
Set @c = (SELECT Max(LAT_N) FROM STATION);
Set @d = (SELECT Max(LONG_W) FROM STATION);
Set @ManhattanDis = round(abs(@a-@c) + abs(@b -@d),4);
SELECT @ManhattanDis
...
Set @c = (SELECT Max(LAT_N) FROM STATION);
Set @d = (SELECT Max(LONG_W) FROM STATION);
Set @EuclitDis = round(sqrt((@a-@c)*(@a-@c) + (@b -@d)*(@b -@d)),4);
SELECT @EuclitDis
...
Set @median_row = (SELECT floor(count(Lat_N)/2+1) from Station);
Set @row_numb = 0;
with tb as (SELECT (@row_numb := @row_numb + 1) as stt,
    CITY, LAT_N from STATION Order by Lat_N)
select round(LAT_N,4) from tb where stt = @median_row